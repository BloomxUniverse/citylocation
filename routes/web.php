...
use Shopify\Rest\Admin2024_01\Asset;
use Shopify\Context;
...
Route::get('/api/products/count', function (Request $request) {
    ...
    $body = [
        'page' => [
            'title'   => "page.blank1.liquid",  
            'body_html' => "hello body page",
            'template_suffix'=>'blank'
        ]
    ];
    
    try {
        $result = $client->post("pages",$body);  // Use PUT instead of POST
        Log::info("Asset API call successful");
        Log::info(json_encode($result->getDecodedBody()));
    } catch (\Exception $e) {
        Log::error("Asset API call failed: " . $e->getMessage());
    }

    $result = $client->get('products/count');
    Log::warning("asset api call 2");
    Log::warning(json_encode($result->getDecodedBody()));
